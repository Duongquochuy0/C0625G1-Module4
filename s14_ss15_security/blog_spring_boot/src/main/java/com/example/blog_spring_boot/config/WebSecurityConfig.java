...
//        http.csrf((csrf) -> csrf
//                        .csrfTokenRepository(CookieCsrfTokenRe`pository.withHttpOnlyFalse())
//                );
        // các đường dẫn không phải login
        http.authorizeHttpRequests(authorize -> authorize
                .requestMatchers("/home", "/blog", "/login","/logout", "/logoutSuccessful", "/403").permitAll()
                .requestMatchers("/blog/create").hasRole("USER")
                .requestMatchers("/admin").hasRole("ADMIN")
                .requestMatchers("/userInfo", "/blog/delete").hasAnyRole("USER", "ADMIN")
                .anyRequest().authenticated() // tất cả các request còn lại phải login
        );
